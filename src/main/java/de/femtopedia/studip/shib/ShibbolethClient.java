...
public class ShibbolethClient extends CustomAccessClient {
    ...
    /**
     * Tries to authenticate with the Shibboleth SSO, if there isn't already an
     * account signed in, and save the required cookies.
     *
    ...
    public void authenticate(String username, String password)
        ...
        try {
            if (!isSessionValid()) {
                authenticateIntern(username, password);
            }
    ...
    /**
     * Tries to authenticate with the Shibboleth SSO and save the required
     * cookies.
     ...
     * @throws IllegalStateException    if cookies don't match the server.
     */
    private void authenticateIntern(String username, String password)
            throws IOException, IllegalArgumentException,
            IllegalAccessException, IllegalStateException {
        CustomAccessHttpResponse response = null;
        try {
            response = get("https://studip.uni-passau.de/studip/index.php");
            addCookie("https://studip.uni-passau.de",
                    createCookie("cache_session",
                            Long.toString(System.currentTimeMillis()),
                            "studip.uni-passau.de", "/"));
            addCookie("https://studip.uni-passau.de",
                    createCookie("navigation-length", "3",
                            "studip.uni-passau.de", "/studip/"));
            response.close();
            ...
            response = post("https://studip.uni-passau.de/Shibboleth.sso"
                            + "/SAML2/POST",
                    new String[]{"Referer"}, new String[]{
                            "https://sso.uni-passau.de" + loc}, formList1);
            response.close();
    ...
    public void addCookie(String url, Cookie cookie) {
        getCookieHelper().setCookie(url, cookie);
    }
     ...
     */
    public Cookie createCookie(String key, String value, String domain,
                               String path) {
